...
export default function BookingsAdminPage() {
  ...
  const handleAssignTutor = async (tutorId) => {
    ...
    try {
      ...
      await adminApi.updateBooking(id, { tutor: tutorId });
      ...
      if (selectedBookingForDetails && bookingId(selectedBookingForDetails) === id) {
        ...
        setSelectedBookingForDetails(prev => ({ ...prev, tutor: updatedTutor }));
      }
  ...
  const confirmed = bookings.filter(b => b.status === 'Confirmed' || b.status === 'Called').length;
  ...
  const tabs = [
    ...
    { id: 'Confirmed', label: 'Confirmed / Called', count: confirmed },
    { id: 'Completed', label: 'Completed', count: completed },
    { id: 'Cancelled', label: 'Declined / Cancelled', count: cancelled },
  ];
  ...
  const filtered = bookings.filter(b => {
    ...
    else if (selectedTab === 'Pending') matchTab = (b.status || 'Pending') === 'Pending';
    else if (selectedTab === 'Confirmed') matchTab = ['Confirmed', 'Called'].includes(b.status || '');
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          { label: 'Pending Callback', value: pending, color: 'text-amber-600 bg-amber-50 border-amber-100', icon: Clock },
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            {(booking.status || 'Pending') === 'Pending' && (
                              <button
                                onClick={() => handleStatus(bookingId(booking), 'Confirmed')}
                            ...
                            {['Pending', 'Confirmed'].includes(booking.status || 'Pending') && (
                              <button
                                onClick={() => handleStatus(bookingId(booking), 'Declined')}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm space-y-4">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex items-center justify-between p-2.5 bg-amber-50 rounded-xl border border-amber-100">
                ...
                <span className="font-extrabold text-amber-900 text-sm">{pending}</span>
              </div>
            </div>
          </div>
